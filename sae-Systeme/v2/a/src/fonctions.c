...
void attendre_processus(){
    int termines = 0; // Nombre de processus terminés
    int status;

    while (termines < NOMBRE_PID) {
        for (int i = 0; i < NOMBRE_PID; i++) {
            if (PIDS[i] != 0) { // Vérifiez uniquement les PIDs encore actifs
                pid_t retour = waitpid(PIDS[i], &status, WNOHANG);
                ...
                if (retour == -1) {
                    ...
                    PIDS[i] = 0; // Marquez le processus comme terminé
                    termines++;
                }
            }
        }

        // Petite pause pour éviter une boucle infinie trop gourmande
